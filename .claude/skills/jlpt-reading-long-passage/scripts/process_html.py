#!/usr/bin/env python3
"""
process_html.py — Pipeline for JLPT "đoạn văn dài" (long-passage) HTML files.
...
Does three things — NO screenshot:
  ...
  3. Append / update rows in the 45-column question_sheet.csv
     — populates 3 câu hỏi (N1) hoặc 4 câu hỏi (N3)

This skill applies only to N1 and N3. Other levels are rejected với UNKNOWN_LEVEL.

Usage
-----
# Count chars only
python3 process_html.py --count-only --file assets/html/doan_van_dai/N1_abcdef.html

# Validate Target Range
python3 process_html.py --validate --html-dir assets/html/doan_van_dai

# Full pipeline — RECOMMENDED: JSON file cho 3-4 questions
python3 process_html.py \
    --file assets/html/doan_van_dai/N1_abcdef.html \
    --csv sheets/samples_v1.csv \
    --tag "phê bình văn hóa" \
    --questions-json /tmp/qs.json

# Full pipeline — CLI flags (nhiều flags vì N3=4 câu)
python3 process_html.py \
    --file assets/html/doan_van_dai/N1_abcdef.html \
    --csv sheets/samples_v1.csv \
    --tag "phê bình văn hóa" \
...
python3 process_html.py --refresh --html-dir assets/html/doan_van_dai --csv sheets/samples_v1.csv
...
# Chỉ N1 & N3 — N2/N4/N5 không có kind này.
TARGET_RANGE = {
    "N1": (1000, 1150),
    "N3": (550, 700),
}
...
def classify_char_count(level: str | None, chars: int) -> str:
    if level not in TARGET_RANGE:
        # N2/N4/N5 không có đoạn văn dài
...
def validate_question_list(level: str | None, questions: list[dict]) -> list[str]:
    ...
    if level not in EXPECTED_Q_COUNT:
        warnings.append(f"Level {level} không thuộc scope đoạn văn dài (chỉ N1/N3).")
...
def cmd_single_full(args) -> None:
    ...
    if status == "UNSUPPORTED_LEVEL":
        print(f"❌ {row['_id']}: level {level} không thuộc đoạn văn dài (chỉ N1/N3). Không commit.")
        sys.exit(1)
    if status == "HARD_REJECT":
...
def main():
    ap = argparse.ArgumentParser(
        description="Process JLPT 'đoạn văn dài' HTML files (no screenshot, 3-4 questions support, N1+N3 only).",
