#!/usr/bin/env python3
"""
Fill Q&A data vào CSV một cách an toàn cho đoạn văn dài (multi-question 3-4 câu) —
tự động handle comma, newline, quoting. Agent BẮT BUỘC dùng script này, không edit
CSV bằng tay.

Đoạn văn dài có 3-4 câu hỏi per bài theo level (CHỈ N1 & N3):
    - N1: 3 câu  → populate question_{1,2,3}, empty question_{4,5}
          câu cuối PHẢI là `question_author_opinion` (outline/main idea)
    - N3: 4 câu  → populate question_{1,2,3,4}, empty question_{5}
          câu cuối PHẢI là `question_content_match` hoặc `question_author_opinion`
...
# Đoạn văn dài CHỈ có N1 và N3
EXPECTED_Q_COUNT = {
    "N1": 3,
    "N3": 4,
}

# Rule câu cuối theo level (spec JLPT)
LAST_LABEL_RULES = {
    "N1": {"question_author_opinion"},  # outline / main idea
    "N3": {"question_content_match", "question_author_opinion"},  # tổng thể
}
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fill Q&A data vào CSV đoạn văn dài (3-4 questions theo level N1/N3)",
    ...
    parser.add_argument(
        ...
        help="N1|N3 — đoạn văn dài CHỈ có 2 level này (không N2/N4/N5)",
    )
    ...
    if len(provided) != expected:
        print(
            ...
            f"   N1: 3 câu (--q1..--q3). N3: 4 câu (--q1..--q4).",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # Validate câu cuối theo spec level
    last_label = labels[-1]
    if last_label not in LAST_LABEL_RULES[level]:
        ...
        print(
            ...
            f"   Lý do: N1 focus 'outline/author's ideas' → câu cuối = author_opinion.\n"
            f"          N3 focus 'summary/logical development' → câu cuối = content_match / author_opinion.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # Đảm bảo general_image luôn rỗng (đoạn văn dài không có screenshot)
