#!/usr/bin/env python3
"""
load_references.py — Load & pretty-print sample JSON của "đoạn văn dài" cho AI agent.

Đọc các file `data/doan_van_dai_n{1,3}_clean.json` (ở project root) và in ra:
  - Stats tổng quan (count, min/max/p25/p50/p75/avg jp_char_count, phân bố Q count/bài)
  - Random samples theo level (để gen agent calibrate style, độ dài, multi-Q)
  - 1 sample cụ thể theo id

Chỉ có N1 và N3 — các level khác KHÔNG CÓ kind "đoạn văn dài".

Usage
-----
python3 load_references.py --stats
python3 load_references.py --level N3 --count 1
...
def data_path(data_dir: str, level: str) -> str:
    n = level.lower()
    return os.path.join(data_dir, f"doan_van_dai_{n}_clean.json")
...
def print_stats(data_dir: str) -> None:
    ...
    print("  N1: 1000-1150  |  N3: 550-700")
    print()
    print("Số câu hỏi CHUẨN (từ rules/question_format.json):")
    print("  N1: 3 câu/bài  |  N3: 4 câu/bài")
    print()
    print("Lưu ý: N2, N4, N5 KHÔNG có kind 'đoạn văn dài'.")
...
def main():
    ap = argparse.ArgumentParser(description="Load JLPT đoạn văn dài reference samples (N1, N3 only).")
    ...
    ap.add_argument("--level", choices=LEVELS, help="Level to load (N1 or N3)")
    ap.add_argument("--count", type=int, default=1, help="Số sample random (default 1 — bài dài)")
